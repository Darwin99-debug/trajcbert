...
while (
    not os.path.exists("/home/daril_kw/data/test/temp_file/checkpoint_epoch_{epoch}.pt")
    and epoch > 0
):
    epoch -= 1
...
def test_autoregressively(
    prediction_dataloader, model, min_traj_rate, target_dict, device
):
    """
    this function will predict the labels of the test set autoregressively
    
    Args:
        prediction_dataloader (torch.utils.data.dataloader.DataLoader): the dataloader of the test set
        model (transformers.modeling_bert.BertForSequenceClassification): the model
        min_traj_rate (float): the rate of the trajectory that we do not predict
        target_dict (dict): the targets dict
        device (torch.device): the device on which we are going to do the predictions
    ...
    # all_predictions = [[]]*len(prediction_dataloader.dataset) but it does not work because it creates a list of the same list then
    all_predictions_detokenized = [
        [] for i in range(len(prediction_dataloader.dataset))
    ]
        ...
        for batch_idx, batch in enumerate(tqdm(prediction_dataloader)):
          # batch 
            ...
            for line in range(len(batch)):
                ...
                first_token_to_predict = first_token_traj + int(
                    min_traj_rate * nb_tokens_traj
                )
                print(
                    f"the number of tokens of the trajectory that we do not predict is {min_traj_rate*nb_tokens_traj}"
                )
                print("the length of the trajectory is ", nb_tokens_traj)
                nb_token_to_predict = nb_tokens_traj - first_token_to_predict
                print(
                    f"we are going to predict the tokens from the point {first_token_to_predict} to the end of the trajectory, ie we predict {nb_token_to_predict} tokens"
                )
                ...
                traj_i_padded = torch.nn.functional.pad( #this function pads the input with 0s so that it has the same length as the input of the model ie 512
                    traj_i, (0, 512 - len(traj_i)), "constant", 0
                )

                # we get the list of the true tokens of the part of the trajectory we are going to predict
                # ie we get the trajectory from the point fisrt_to_predict to the end of the trajectory
                list_true_tokens = traj[first_token_to_predict + first_token_traj :]

                print(
                    f"the length of the list of true tokens is {len(list_true_tokens)}"
                )
                print(f"the nb of tokens to predict is {nb_token_to_predict}")
                assert len(list_true_tokens) == nb_token_to_predict
                # we get the numbers instead of the tensors
                list_true_tokens = list_true_tokens.tolist()
                # we get the ids of the true tokens
                list_true_tokens_ids = [
                    target_dict[token] for token in list_true_tokens
                ]
                ...
                for index_token_to_predict in range(
                    first_token_to_predict, nb_tokens_traj
                ):
                    print(
                        f"we predict the token number {index_token_to_predict} on {nb_tokens_traj}"
                    )
                    ...
                    att_mask = torch.cat(
                        (torch.ones(len(traj_i)), torch.zeros(512 - len(traj_i)))
                    ).to(device)
                    ...
                    print(
                        f"the index of the token is {index_token_to_predict-first_token_to_predict}"
                    )
                    print(
                        f"tis is the {index_token_to_predict-first_token_to_predict}th token to predict and the {index_token_to_predict} token of the trajectory knowing that the length of the trajectory is {nb_tokens_traj}"
                    )
                    target = torch.tensor(
                        [
                            list_true_tokens_ids[
                                index_token_to_predict - first_token_to_predict
                            ]
                        ]
                    ).to(device)
                    # we get the outputs of the model
                    outputs = model(
                        input_ids=traj_i_padded,
                        token_type_ids=None,
                        attention_mask=att_mask,
                        # labels=target, #we do not need the labels because we are going to predict autoregressively
                    ...
                    predicted_token = np.argmax(
                        logits
                    )  # the preicted token is the id in the targets_dict
                    ...
                    for i in range(len(all_predictions[batch_idx])):
                        ...
                        traj_i_padded = torch.cat(
                            (
                                traj_i_padded,
                                torch.tensor([all_predictions[batch_idx][i]]),
                            )
                        )

                    traj_i_padded = torch.nn.functional.pad(
                        traj_i_padded, (0, 512 - len(traj_i_padded)), "constant", 0
                    )
    ...
    for i in range(len(all_predictions_detokenized)):
        scores_coord.append(
            np.median(
                [
                    np.abs(
                        all_predictions_detokenized[i][j][0]
                        - all_true_labels_coord[i][j][0]
                    )
                    + np.abs(
                        all_predictions_detokenized[i][j][1]
                        - all_true_labels_coord[i][j][1]
                    )
                    for j in range(len(all_predictions_detokenized[i]))
                ]
            )
        )
...
def main():
    ...
    (
        ...
        all_predictions_detokenized,
        all_true_labels,
    ) = test_autoregressively(
        prediction_dataloader, model, min_traj_rate, targets_dict, device
    )
