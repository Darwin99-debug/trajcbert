    ...
    for batch_idx, batch in enumerate(tqdm(prediction_dataloader)):
      ...
      #we are going to predict the trajectory named traj autoregressively from the point nb_tokens to the end of the trajectory
        ...
        for index_token_to_predict in range(first_token_to_predict,nb_tokens_traj):
          ...
          print(f"the index of the token is {index_token_to_predict-first_token_to_predict}")
          print(f"tis is the {index_token_to_predict-first_token_to_predict}th token to predict and the {index_token_to_predict} token of the trajectory knowing that the length of the trajectory is {nb_tokens_traj}")
          target=torch.tensor([list_true_tokens_ids[index_token_to_predict-first_token_to_predict]]).to(device)
